 ...
 * @author ThePoisoned1
 */
public class E113b_SemaforosSinParar {
    ...
    public boolean comprobador(int llegarAlUltimo) {
        ...
        for (int i = 0; i < numSemaforos - 1; i++) {
            tiempo = llegarAlUltimo * (avenida[i].x / distanciaTotal); //lo que tardas en llegar al semaforo
            cantCiclos = ((int) tiempo) / avenida[i].ciclo;//ciclos que hace ese semaforo
            tiempo -= cantCiclos * avenida[i].ciclo;//le restas la cantidad de ciclos completos
            if (tiempo < (avenida[i].tiempoRojo - 0.01) && tiempo > 0.01) {//si cuando pasa esta en rojo
                return false;
            }
        }
        return true;
    ...
    public boolean caso() {
        ...
        avenida = new Semaforo[numSemaforos - 1];
    ...
    public static void main(String[] args) {
        new E113b_SemaforosSinParar().run();
    }
}
